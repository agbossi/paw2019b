    ...
    <body>
        <c:forEach var="appointment" items="${patient.appointments}">
            <p><format:formatDate value="${appointment.date.getTime()}" type="date" pattern="EEEE dd/MM/yyyy HH:mm"/>
                ...
                <a class="btn btn-outline-primary" href="/cancelApp/${appointment.doctorClinic.clinic.id}/${appointment.doctorClinic.doctor.license}/${appointment.date.get(1)}-${appointment.date.get(2)}-${appointment.date.get(5)}-${appointment.date.get(11)}"><spring:message code="a.cancel"/></a>
            </p>
        </c:forEach>
    </body>
</html>